...
class AiSolutionKnowledgeTests(unittest.TestCase):
    def test_public_solution_becomes_parameterized_semantic_template(self):
        ...
        self.assertIn("95%", parameter_by_name["品牌核心类目"]["description"])
    ...
    def test_status_reports_loaded_solution_count_and_latest_update(self):
        ...
        self.assertEqual(status["storeCategoryProfileCount"], 204)
        self.assertEqual(status["fullyMappedStoreCategoryProfileCount"], 140)
        self.assertEqual(status["storeCategoryProfileMonths"], ["2026-06", "2026-07"])

    def test_store_sales_profile_resolves_brand_alias_and_keeps_95_percent_cut(self):
        ...
        self.assertEqual(profile["coreCategoryCount"], 9)
        ...
        self.assertEqual(len(knowledge.brand_core_categories("兰蔻")), 8)
