...
Gallery White 已完成纯白三栏、无灰下拉、独立胶囊和参数精确联动。用户在 `1538×674` 实机视图继续标注了 11 个问题，集中在三类体验：左侧组件库边框和控件过密；页面重复解释文案过多；胶囊及任务按钮文字没有稳定双轴居中。另有一项明确行为调整：取消数据引擎人群圈包根据节点参数自动生成人群包名称。
...
- 左侧行为组件库采用 A「无框行列表」。
- 新建或清空数据引擎人群圈包时，人群包名称为空，只由用户手动输入。
...
| 3 | 数据引擎人群圈包标题解释 | 删除“自由搭建当前画布，并可直接存为方案草稿”。 |
| 4 | 顶部三个胶囊文字未居中 | 三个导航内部统一 `inline-flex` 双轴居中和稳定 `line-height: 1`。 |
| 5 | 数据引擎取数模板解释 | 删除“草稿编辑、发布与数据引擎人群圈包预览”。 |
...
| 10 | 达摩盘取数仍有灰色按钮 | 所有达摩盘取数按钮的 disabled 表面改为纯白，使用边框、文字色和 cursor 表达禁用。 |
| 11 | 达摩盘取数按钮文字居中 | `运行`、`显示字段`、`Rebase` 等按钮统一双轴居中，不依赖默认行高。 |
...
- `SolutionCenter.vue`：草稿编辑、发布与数据引擎人群圈包预览；创建字段，让一个字段控制多个组件。
...
- watcher 继续负责数据引擎人群圈包字段约束、JSON 防抖生成和快照，不改变其他消费者。
...
- 不发送新请求，不修改请求结构或后端验证。
- 不修改方案序列化字段，仅改变数据引擎人群圈包可见名称的初始化和自动更新来源。
...
- 去灰契约：达摩盘取数全部 disabled 按钮及其 hover 使用 `--ui-surface`，不使用 `--ui-fill` 或灰色字面背景；具体运行和设置按钮规则不得被旧样式覆盖。
