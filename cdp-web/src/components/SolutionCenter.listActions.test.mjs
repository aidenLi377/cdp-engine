...
test('solution editor toolbar separates copy from draft-only editing controls', () => {
  ...
  assert.match(solutionCenterVue, /const canManagePublicSolutions = computed\(\(\) => props\.currentUserRole === 'super_admin'\)/)
  assert.match(solutionCenterVue, /const isReadOnly = computed\(\s*\(\) =>/)
  assert.match(solutionCenterVue, /libraryScope\.value === 'public' && !canManagePublicSolutions\.value/)
